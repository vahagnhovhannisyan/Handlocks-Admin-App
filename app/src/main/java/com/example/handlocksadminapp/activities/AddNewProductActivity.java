...
public class AddNewProductActivity extends AppCompatActivity {
    ...
    private void createProducts() {
        ...
        firestore.collection("NewProducts").add(cartMap).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
            @Override
            public void onComplete(@NonNull Task<DocumentReference> task) {
                if(task.isSuccessful()) {

                    progressBar.setVisibility(View.GONE);
                    Toast.makeText(AddNewProductActivity.this, "Նոր Ապրանքը Ստեղծված է", Toast.LENGTH_SHORT).show();
                }
                else{
                    Toast.makeText(AddNewProductActivity.this, "Սխալ։" + task.getException(), Toast.LENGTH_SHORT).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 100 && data != null && data.getData() != null ){
            ...
            storageReference = FirebaseStorage.getInstance().getReference("new_products/" + fileName);
